<%@ Page Title="Process Safety Program Details | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/Home_{{VER}}/ClientConnectServerLocationDetail.Master" CodeBehind="locationdetails.aspx.vb" Inherits="GRC.Connect.Web.locationdetails_processsafety" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div id="tabLocDetailsContainer" class="tabLocDetailsContainer">
        ...
        <div class="choosePracticeHldr">
            ...
            <div id="procSafetyPracticeLvlDropDown"></div>
        </div>
            ...
            var view = {
                ...
                loadActiveServicesSuccess: function (pResults) {
                    try {
                        if (pResults.isSuccess) {
                            ...
                            if (!view.procSafetyPracticeLvlDropDown) {
                                $('#procSafetyPracticeLvlDropDown').kendoDropDownList({
                            ...
                            view.procSafetyPracticeLvlDropDown = $('#procSafetyPracticeLvlDropDown').data("kendoDropDownList");
                            view.procSafetyPracticeLvlDropDown.select(function (dataItem) { return dataItem.value === "57" });
                        }
                    } catch (pException) {
                ...
                serviceDDChange: function (e) {
                    ...
                    switch (chosenServiceValue) {
                        case "51":
                            //B&M
                            tempURL = currentURL.replace("/ProcessSafety/", "/Boiler/");
                        ...
                        case "52":
                            //FIRE
                            tempURL = currentURL.replace("/ProcessSafety/", "/Fire/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "53":
                            //IR
                            tempURL = currentURL.replace("/ProcessSafety/", "/Infrared/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "54":
                            //DIVSERV
                            tempURL = currentURL.replace("/ProcessSafety/", "/Diversified/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "55":
                            //NATHAZ
                            tempURL = currentURL.replace("/ProcessSafety/", "/NaturalHazards/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "56":
                            //JURIS
                            tempURL = currentURL.replace("/ProcessSafety/", "/Jurisdictional/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "57":
                            //PROCSAFETY
                            tempURL = currentURL.replace("/ProcessSafety/", "/ProcessSafety/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
