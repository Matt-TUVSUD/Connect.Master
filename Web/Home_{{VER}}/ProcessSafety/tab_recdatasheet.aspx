<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="tab_recdatasheet.aspx.vb" Inherits="GRC.Connect.Web.tab_recdatasheet_processsafety" %>
    ...
    $(document).ready(function () {
        ...
        var view_RecDatasheet = {
            ...
            onExport: function () {
                ...
                if (view_RecDatasheet.clientFeatures().indexOf("Q") > -1) {
                    Header = buildXMLRecHeader_BoilerMgmt();
                    Content = buildXMLString_BoilerMgmt(gridRecDatasheet.dataSource._view, view_RecDatasheet.objListing());
                    Footer = buildXMLRecFooter();
                } else {
                    Header = buildXMLRecHeader_Boiler();
                    Content = buildXMLString_Boiler(gridRecDatasheet.dataSource._view, view_RecDatasheet.objListing());
                    Footer = buildXMLRecFooter();
                }
            ...
            loadFileNumber: function () {
                ...
                //if (!view_RecDatasheet.collListing.ViewCCLocationPSRecommendationCollections(tempParm)) {
                if (!view_RecDatasheet.collListing.ViewCCLocationPSCollections(tempParm)) {
                    var tempHemisphere = getHemisphere(calculate_time_zone());
                    es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                    if (!view_RecDatasheet.collRec.viewCCPSRecommendationGridCollection(view_RecDatasheet.gSafeGuid + "|" + tempHemisphere)) {
        ...
        if (view_RecDatasheet.clientFeatures().indexOf("Q") > -1) {
            gridRecDatasheet = $("#gridRecDatasheet").kendoGrid({
                ...
                { field: "RespDate", width: "75px", title: "Resp. Date", template: "#= (RespDate == null || RespDate == undefined || RespDate == '' || RespDate == 'Mon Jan 1 00:00:00 EST 1900' || RespDate.toString().indexOf('1900') > -1 || RespDate.toString().indexOf('1899') > -1) ? ' ' : kendo.toString(RespDate, 'dd-MMM-yy') #" },
                ...
                { field: "ExpdComplDate", width: "125px", title: "Expd. Compl. Date", template: "#= (ExpdComplDate == null || ExpdComplDate == undefined || ExpdComplDate == '' || ExpdComplDate == 'Mon Jan 1 00:00:00 EST 1900' || ExpdComplDate.toString().indexOf('1900') > -1 || ExpdComplDate.toString().indexOf('1899') > -1) ? ' ' : kendo.toString(ExpdComplDate, 'dd-MMM-yy') #" },
