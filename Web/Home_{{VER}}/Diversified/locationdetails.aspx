<%@ Page Title="Diversified Program Details | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/Home_{{VER}}/ClientConnectServerLocationDetail.Master" CodeBehind="locationdetails.aspx.vb" Inherits="GRC.Connect.Web.locationdetails_diversified" %>
    ...
    <div id="tabLocDetailsContainer" class="tabLocDetailsContainer">
        <div id="tabLocDetailsLoading" class="tabLocDetailsLoading">
         ...
         <div class="choosePracticeHldr">
            ...
            <div id="divservPracticeLvlDropDown"></div>
        </div>
            ...
            var view = {
                ...
                loadActiveServicesSuccess: function (pResults) {
                    try {
                        if (pResults.isSuccess) {
                            ...
                            if (!view.divservPracticeLvlDropDown) {
                                $('#divservPracticeLvlDropDown').kendoDropDownList({
                            ...
                            view.divservPracticeLvlDropDown = $('#divservPracticeLvlDropDown').data("kendoDropDownList");
                            view.divservPracticeLvlDropDown.select(function (dataItem) { return dataItem.value === "54" });
                        }
                    } catch (pException) {
                ...
                serviceDDChange: function (e) {
                    ...
                    switch (chosenServiceValue) {
                        case "51":
                            //B&M
                            tempURL = currentURL.replace("/Diversified/", "/Boiler/");
                        ...
                        case "52":
                            //FIRE
                            tempURL = currentURL.replace("/Diversified/", "/Fire/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "53":
                            //IR
                            tempURL = currentURL.replace("/Diversified/", "/Infrared/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "54":
                            //DIVSERV
                            tempURL = currentURL.replace("/Diversified/", "/Diversified/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "55":
                            //NATHAZ
                            tempURL = currentURL.replace("/Diversified/", "/NaturalHazards/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "56":
                            //JURIS
                            tempURL = currentURL.replace("/Diversified/", "/Jurisdictional/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "57":
                            //PROCSAFETY
                            tempURL = currentURL.replace("/Diversified/", "/ProcessSafety/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
