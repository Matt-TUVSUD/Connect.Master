<%@ Page Title="Natural Hazards Details | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/Home_{{VER}}/ClientConnectServerLocationDetail.Master" CodeBehind="locationdetails.aspx.vb" Inherits="GRC.Connect.Web.locationdetails_nathaz" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div id="tabLocDetailsContainer" class="tabLocDetailsContainer">
        ...
        <div class="choosePracticeHldr">
            ...
            <div id="natHazPracticeLvlDropDown"></div>
        </div>
            ...
            var view = {
                ...
                loadActiveServicesSuccess: function (pResults) {
                    try {
                        if (pResults.isSuccess) {
                            ...
                            if (!view.natHazPracticeLvlDropDown) {
                                $('#natHazPracticeLvlDropDown').kendoDropDownList({
                            ...
                            view.natHazPracticeLvlDropDown = $('#natHazPracticeLvlDropDown').data("kendoDropDownList");
                            view.natHazPracticeLvlDropDown.select(function (dataItem) { return dataItem.value === "55" });
                        }
                    } catch (pException) {
                ...
                serviceDDChange: function (e) {
                    ...
                    switch (chosenServiceValue) {
                        case "51":
                            //B&M
                            tempURL = currentURL.replace("/NaturalHazards/", "/Boiler/");
                        ...
                        case "52":
                            //FIRE
                            tempURL = currentURL.replace("/NaturalHazards/", "/Fire/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "53":
                            //IR
                            tempURL = currentURL.replace("/NaturalHazards/", "/Infrared/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "54":
                            //DIVSERV
                            tempURL = currentURL.replace("/NaturalHazards/", "/Diversified/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "55":
                            //NATHAZ
                            tempURL = currentURL.replace("/NaturalHazards/", "/NaturalHazards/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "56":
                            //JURIS
                            tempURL = currentURL.replace("/NaturalHazards/", "/Jurisdictional/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "57":
                            //PROCSAFETY
                            tempURL = currentURL.replace("/NaturalHazards/", "/ProcessSafety/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
