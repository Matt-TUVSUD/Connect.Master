...
function utilKendoGrid() {
    ...
    this.optionsConstant = {
        ...
        detailTemplate: null,
        criteriaFilter: null,
        selectedRecsOnly: false,
        sortIndex: 1,
        gridFilter: "",
        gridSort: "",
        dataBoundFunction: null,
        sortFunction: null
    };
    ...
    /// A template for the options object for server side loading of WHAT IF application
    this.WI_setOptionsTemplate_ServerLoading = function () {
        var zParent = this;
        var zHeight = zParent._getAvailableHeight();
        var zReturn = {
            dataSource: {
                ...
                transport: {
                    read: {
                        ...
                        complete: function (pEvent, pStatus) {
                            //debugger;
                            var responseJSONdata = pEvent.responseJSON;
                            ...
                            if (responseJSONdata.Filter) { zParent.filterString = responseJSONdata.Filter; }
                            if (responseJSONdata.Sort) { zParent.sortString = responseJSONdata.Sort; }
                            if (responseJSONdata.Pin) { zParent.lockedString = responseJSONdata.Pin; }
                    ...
                    parameterMap: function (pData, pType) {
                        ...
                        if (pType === "read") {
                            ...
                            if (!pData["sort"] || pData["sort"].length < 1) {
                                pData["sort"] = null;
                            } else {
                            ...
                            return {    
                                filter: zParent.optionsConstant.gridFilter,
                                sort: pData.sort,
                                take: pData.take,
                                skip: pData.skip,
                                page: pData.page,
                                pageSize: pData.pageSize,
                                sortIndex: zParent.optionsConstant.sortIndex,
                                selected: zParent.optionsConstant.selectedRecsOnly
                            }
                        }
                    }
                },
            ...
            dataBound: function(e){
                zParent.optionsConstant.dataBoundFunction(e)
            },
            change: function (pEvent) {
                zParent.optionsConstant.changeFunction(pEvent);
            },
            ...
            sort: function (e) {
                zParent.optionsConstant.gridSort = JSON.stringify(e.sort);
            },
            scrollable: {
                virtual: true
            },
    ...
    /// WHAT IF APPLICATION INITGRID FUNCTION
    /// Initialize the Kendo grid helper class FOR THE WHAT IF APPLICATION.
    ...
    /// pColumnIndices
    /// pColumnSources
    this.initGridWI = function (pGridDOM, pGridContainerDOM, pPickListDOM, pData, pSchema, pColumns, pIsEditMode, pSort, pFilter, pPage, pColumnIndices, pColumnSources) {
        ...
        this._setupGridWI(this._gridDOM, pData, pSchema, pColumns, pIsEditMode, pSort, pFilter, pPage);
    ...
    this._setupGrid = function (pGridDOM, pData, pSchema, pColumns, pIsEditMode, pSort, pFilter, pPage) {
        ...
        if (pIsEditMode) {
            if (zParent.optionsConstant.isServerLoading) {
                zOptions = zParent.WI_setOptionsTemplate_ServerLoading();
            } else {
    ...
    /// Build the Kendo WHAT IF grid and pass information to the grid settings.
    /// pGridDOM (STRING) Id of the DIV where the grid will be in HTML. Format requires # at beginning. REQUIRED
    /// pData (JSON OBJECT) the data for the grid. REQUIRED
    /// pSchema (JSON OBJECT) The column schema for the grid. OPTIONAL
    /// pColumns (JSON OBJECT) The column list for the grid. OPTIONAL
    ...
    /// pFilter (JSON OBJECT) object indicating pre-filter. OPTIONAL
    /// pPage (JSON OBJECT) object indicating the page to be set. OPTIONAL
    this._setupGridWI = function (pGridDOM, pData, pSchema, pColumns, pIsEditMode, pSort, pFilter, pPage) {
        var zParent = this;
        var zOptions = null;
        if (pIsEditMode) {
            if (zParent.optionsConstant.isServerLoading) {
    ...
    this._setupColumnMenuMap = function (pEvent, pParent) {
        ...
        var zIconClassSquare = "fa-square-o";
        var zIconClassChecked = "fa-check-square-o"; //"fa-check"; //"fa-check-square-o";
        var zIconOff = '<i style="font-size: 9pt; margin-right: 14px;" class="fa ' + zIconClassSquare + '"></i>';
        var zIconOn = '<i style="font-size: 9pt; margin-right: 12px;" class="fa ' + zIconClassChecked + '"></i>';
    ...
    this._setupPickList = function (pField) {
        ...
        if (this.optionsConstant.isServerLoading) {
            ...
            var zPickListURL = this.optionsConstant.transportPickListAddress + '?field=' + pField + '&filter='+  encodeURI(zFilter)
            global.callWIRestService(zPickListURL, this._setupPickListServerLoadingSuccess, this._setupPickListServerLoadingError, this)
    ...
    this.setFilterPicklist = function (pFilter, pField) {
        ...
        if (zFilter != undefined) {
            ...
            this.optionsConstant.gridFilter = encodeURI(JSON.stringify(zFilter));
            this.grid.dataSource.page(zPage);
        } else {
            this.grid.dataSource.filter(pFilter);
            this.optionsConstant.gridFilter = encodeURI(JSON.stringify(pFilter));
            this.grid.dataSource.page(zPage);
        }
    };
    ...
    /// THIS FUNCTION HAS BEEN ALTERED FOR USE WITH THE WHAT IF TOOL ///
    ...
    this.rebuildGrid = function (pIsEditMode, pSchema, pColumns, pData, pSort, pFilter, pPage, pColumnIndices, pColumnSources) {
        ...
        this._setupGridWI(this._gridDOM, zData, zSchema, zColumns, pIsEditMode);
