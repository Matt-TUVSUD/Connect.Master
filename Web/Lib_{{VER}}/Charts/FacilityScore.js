$(document).ready(function () {
    ...
    if (ServerSide_IsBatchMode == "true") {
        globalClientFeatures = ServerSide_ClientFeatures;
    }
    else {
    ...
    viewChart = {
        ...
        chartLabelFunctionName: "wrapChartLabels",
        chartLabelMaxLength: "65",
        chartRatingMax: 0,
        ...
        averageFiltered: ko.observable(),

        //MAGNA SPECIFIC OBSERVABLES
        magnaClientAverages: ko.observable(),
        magnaClientAveragesWeighted: ko.observable(),
        magnaClientAchievableAverages: ko.observable(),
        magnaClientAchievableWeightedAverages: ko.observable(),

        magnaCustomAccessLabel: ko.observable(),
        magnaCustomAccessAverages: ko.observable(),
        magnaCustomAccessWeightedAverages: ko.observable(),
        magnaCustomAccessAchievableAverages: ko.observable(),
        magnaCustomAccessAchievableWeightedAverages: ko.observable(),

        magnaDivisionAverages: ko.observable(),
        magnaDivisionWeightedAverages: ko.observable(),
        magnaDivisionAchievableAverages: ko.observable(),
        magnaDivisionAchievableWeightedAverages: ko.observable(),

        magnaFilteredAverages: ko.observable(),
        magnaFilteredWeightedAverages: ko.observable(),
        magnaFilteredAchievableAverages: ko.observable(),
        magnaFilteredAchievableWeightedAverages: ko.observable(),

        magnaIndustryLabel: ko.observable(),
        magnaIndustryAverages: ko.observable(),
        magnaIndustryWeightedAverages: ko.observable(),
        magnaIndustryAchievableAverages: ko.observable(),
        magnaIndustryAchievableWeightedAverages: ko.observable(),

        magnaGRCAverages: ko.observable(),
        magnaGRCWeightedAverages: ko.observable(),
        magnaGRCAchievableAverages: ko.observable(),
        magnaGRCAchievableWeightedAverages: ko.observable(),
        //END MAGNA OBSERVABLES
        ...
        fullMag: ko.observable(false),

        achievableGRCAverage: ko.observable(),
        achievableClientAverage: ko.observable(),
        achievableFilteredAverage: ko.observable(),
        ...
        buildChartData: function () {
            ...
            if (dsChartData.fldId == 57 || dsChartData.fldId == 58) {
                ...
                viewChart.chartLabelFunctionName = "wrapChartLabelsByLocation";
                viewChart.chartLabelMaxLength = "25";
            }
            // CORRECT ANY MISSING TITLES
            if (dsChartData.series[0].data) {
                for (var x = 0; x < dsChartData.series[0].data.length; x++) {
                    //CHECK FOR MAGNA SPECIAL FEATURE
                    var tempValue = "";
                    var nodeLbl = "valueDisplay";
                    if (globalClientFeatures.search('U') > -1) {
                        tempValue = dsChartData.series[0].data[x].fldValue + '&nbsp;(' + dsChartData.series[0].data[x].fldAchievableScorePercentString + ')';
                    } else {
                        tempValue = dsChartData.series[0].data[x].fldValue
                    }
                    dsChartData.series[0].data[x][nodeLbl] = tempValue;
            ...
            //CORRECT SECOND DATA SERIES FOR MAGNA DATA
            //if (globalClientFeatures.search('U') > -1) {
            if (dsChartData.series[1].data) {
                for (var x = 0; x < dsChartData.series[1].data.length; x++) {
                    var tempValue = "";
                    var nodeLbl = "valueDisplay";
                    tempValue = dsChartData.series[1].data[x].fldValue;
                    dsChartData.series[1].data[x][nodeLbl] = tempValue;
                }
            }
            //}
            // IF SERIES DATA COUNT IS LESS THAN DATA ALLOWED ON PAGE, THEN ADD EMPTY ROWS

            /* 9-24-20: SECTION COMMENTED AS IT WAS DETERMINED TO NOT STUFF EMPTY ROWS INTO DATASETS FOR CHARTS */
            ...
            viewChart.averageFiltered(dsChartData.fldFilteredAverages);

            viewChart.magnaClientAverages(dsChartData.fldClientAverages);
            viewChart.magnaClientAveragesWeighted(dsChartData.fldClientAveragesWeighted);
            viewChart.magnaClientAchievableAverages(dsChartData.fldClientAveragesAchievable);
            viewChart.magnaClientAchievableWeightedAverages(dsChartData.fldClientAveragesAchievableWeighted);

            viewChart.magnaCustomAccessLabel(dsChartData.fldCustomAccessLabel);
            viewChart.magnaCustomAccessAverages(dsChartData.fldCustomAccessAverages);
            viewChart.magnaCustomAccessWeightedAverages(dsChartData.fldCustomAccessAveragesWeighted);
            viewChart.magnaCustomAccessAchievableAverages(dsChartData.fldCustomAccessAveragesAchievable);
            viewChart.magnaCustomAccessAchievableWeightedAverages(dsChartData.fldCustomAccessAveragesAchievableWeighted);

            viewChart.magnaDivisionAverages(dsChartData.fldDivisionAverages);
            viewChart.magnaDivisionWeightedAverages(dsChartData.fldDivisionAveragesWeighted);
            viewChart.magnaDivisionAchievableAverages(dsChartData.fldDivisionAveragesAchievable);
            viewChart.magnaDivisionAchievableWeightedAverages(dsChartData.fldDivisionAveragesAchievableWeighted);

            viewChart.magnaFilteredAverages(dsChartData.fldFilteredAverages);
            viewChart.magnaFilteredWeightedAverages(dsChartData.fldFilteredAveragesWeighted);
            viewChart.magnaFilteredAchievableAverages(dsChartData.fldFilteredAveragesAchievable);
            viewChart.magnaFilteredAchievableWeightedAverages(dsChartData.fldFilteredAveragesAchievableWeighted);

            viewChart.magnaIndustryLabel(dsChartData.fldIndustryLabel);
            viewChart.magnaIndustryAverages(dsChartData.fldIndustryAverages);
            viewChart.magnaIndustryWeightedAverages(dsChartData.fldIndustryAveragesWeighted);
            viewChart.magnaIndustryAchievableAverages(dsChartData.fldIndustryAveragesAchievable);
            viewChart.magnaIndustryAchievableWeightedAverages(dsChartData.fldIndustryAveragesAchievableWeighted);

            viewChart.magnaGRCAverages(dsChartData.fldGRCAverages);
            viewChart.magnaGRCWeightedAverages(dsChartData.fldGRCAveragesWeighted);
            viewChart.magnaGRCAchievableAverages(dsChartData.fldGRCAveragesAchievable);
            viewChart.magnaGRCAchievableWeightedAverages(dsChartData.fldGRCAveragesAchievableWeighted);

            viewChart.chartId = dsChartData.fldId;
            if (globalClientFeatures.search('U') > -1) {
                if (viewChart.chartId == 57) {
                    viewChart.fullMag(true);
                } else if (viewChart.chartId == 4) {
                    viewChart.fullMag(false);
                }
            }
            ...
            viewChart.chartPracticeName = dsChartData.fldTitlePageSub;
            ...
            try {
                ...
                if (viewChart.chartRating == null) {
                    $("#chartRating").kendoChart({
                        ...
                        legend: {
                            ...
                            item: {
                                visual: function (pEvent) {
                                    // CREATE A COLORED SQUARE WITH OUTLINE ON LEGEND
                                    try {
                                        //debugger;
                                        // GATHER INFORMATION
                                        if (pEvent) {
                                            if (pEvent.series.data.length > 0) {
                                                var zColor = pEvent.series.data[0].fldColor; // pEvent.options.markers.background;
                                            } else {
                                            ...
                                            var zPos = [0, 0];
                                            if (globalClientFeatures.search('U') > -1) {
                                                if (pEvent.series.index == 0) {
                                                    var tempLbl = pEvent.series.name;
                                                    tempLbl = tempLbl + " - (Achievable Rating %)";
                                                    var zLabel = new kendo.drawing.Text(tempLbl, zPos, {
                                                        fill: {
                                                            color: zLabelColor
                                                        }
                                                    });
                        ...
                        categoryAxis: {
                            labels: {
                                template: "#= " + viewChart.chartLabelFunctionName + "(value, " + viewChart.chartLabelMaxLength + ")#"
                            },
                            majorGridLines: {
                                visible: false,
                                color: "#111111"
                        ...
                        seriesDefaults: {
                            ...
                            labels: {
                                ...
                                template: function (e) { if (e.dataItem) { return e.dataItem.fldValue; } else { return ""; } }
                            }
                        },
                        tooltip: {
                            visible: true,
