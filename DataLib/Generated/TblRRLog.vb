...
Namespace razor
    Partial Public Class TblRRLog

        Public Shared Sub Delete(ByRef pRowData As IrowTblRRLog)
            Delete(pRowData, Nothing, Nothing)
        End Sub

        Public Shared Sub Delete(ByRef pRowData As IrowTblRRLog, pDeleteEventUserState As Object)
            Delete(pRowData, Nothing, pDeleteEventUserState)
        End Sub

        Public Shared Sub Delete(ByRef pRowData As IrowTblRRLog, ByRef pTransaction As SqlClient.SqlTransaction)
            Delete(pRowData, pTransaction, Nothing)
        End Sub

        Public Shared Sub Delete(ByRef pRowData As IrowTblRRLog, ByRef pTransaction As SqlClient.SqlTransaction, pDeleteEventUserState As Object)
            Dim zSQL As String = ""
            Dim zMode As String = ""
        ...
        Dim zObj As rowTblRRLog = LoadByPrimaryKey(pcolId, pTransaction, Nothing)
        Delete(zObj, pTransaction, pDeleteEventUserState)
        End Sub	

        Public Shared Sub Save(ByRef pRowData As rowTblRRLog)
            Save(pRowData, Nothing, Nothing)
        End Sub

        Public Shared Sub Save(ByRef pRowData As rowTblRRLog, ByRef pTransaction As SqlClient.SqlTransaction)
            Save(pRowData, pTransaction, Nothing)
        End Sub

        Public Shared Sub Save(ByRef pRowData As rowTblRRLog, ByRef pTransaction As SqlClient.SqlTransaction, pSavedEventUserState As Object)
		...
		If pRowData.IsDirty() Then
            ...
            If razor.UtilSQLServer.IsKeySet(pRowData.colId) Then
                ...
                zSQL = "SELECT COUNT(*) from [dbo].[RRLog] WHERE [ID] = @colId"
            ...
            Select Case zMode
                Case "UPDATE"
                    If pRowData.IsDirty Then
                        ...
                        zUpdate = "UPDATE [dbo].[RRLog] SET " 
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colFilePrefix) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [FilePrefix] = @colFilePrefix "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colFileNo) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [FileNo] = @colFileNo "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colPracticeCode) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [PracticeCode] = @colPracticeCode "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colHazard) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [Hazard] = @colHazard "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colDateStamp) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [DateStamp] = @colDateStamp "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colSender) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [Sender] = @colSender "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRecipientName) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RecipientName] = @colRecipientName "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRecipientEmail) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RecipientEmail] = @colRecipientEmail "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colActivity) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [Activity] = @colActivity "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActive) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [LinkActive] = @colLinkActive "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedDate) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [LinkActivatedDate] = @colLinkActivatedDate "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedBy) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [LinkActivatedBy] = @colLinkActivatedBy "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedByEmail) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [LinkActivatedByEmail] = @colLinkActivatedByEmail "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colResponseActive) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [ResponseActive] = @colResponseActive "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colResponseLastActivity) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [ResponseLastActivity] = @colResponseLastActivity "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colChanged) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [Changed] = @colChanged "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU1Date) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RFU1Date] = @colRFU1Date "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU2Date) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RFU2Date] = @colRFU2Date "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU3Date) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RFU3Date] = @colRFU3Date "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colRRLogGuid) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [RRLogGuid] = @colRRLogGuid "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colCurrentLink) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [CurrentLink] = @colCurrentLink "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colLanguagePreference) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [LanguagePreference] = @colLanguagePreference "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colAdminAlertDate) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [AdminAlertDate] = @colAdminAlertDate "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colAdminAlertReason) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [AdminAlertReason] = @colAdminAlertReason "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysUserCreated) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [SysUserCreated] = @colSysUserCreated "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysDateCreated) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [SysDateCreated] = @colSysDateCreated "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysUserModified) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
                            zSet = zSet & " [SysUserModified] = @colSysUserModified "
                        End If
                        If pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysDateModified) Then 
                            If zSet.Length > 0 Then zSet = zSet & ","
            ...
            End If
                ...
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colFilePrefix) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colFileNo) Then
                    zCmd.Parameters.Add(New SqlParameter("@colFileNo", IIf(pRowData.colFileNo Is Nothing, DBNull.Value, pRowData.colFileNo)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colPracticeCode) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colHazard) Then
                    zCmd.Parameters.Add(New SqlParameter("@colHazard", IIf(pRowData.colHazard Is Nothing, DBNull.Value, pRowData.colHazard)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colDateStamp) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colSender) Then
                    zCmd.Parameters.Add(New SqlParameter("@colSender", IIf(pRowData.colSender Is Nothing, DBNull.Value, pRowData.colSender)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRecipientName) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRecipientEmail) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colActivity) Then
                    zCmd.Parameters.Add(New SqlParameter("@colActivity", IIf(pRowData.colActivity Is Nothing, DBNull.Value, pRowData.colActivity)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActive) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedDate) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedBy) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colLinkActivatedByEmail) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colResponseActive) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colResponseLastActivity) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colChanged) Then
                    zCmd.Parameters.Add(New SqlParameter("@colChanged", IIf(pRowData.colChanged Is Nothing, DBNull.Value, pRowData.colChanged)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU1Date) Then
                    zCmd.Parameters.Add(New SqlParameter("@colRFU1Date", IIf(pRowData.colRFU1Date Is Nothing, DBNull.Value, pRowData.colRFU1Date)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU2Date) Then
                    zCmd.Parameters.Add(New SqlParameter("@colRFU2Date", IIf(pRowData.colRFU2Date Is Nothing, DBNull.Value, pRowData.colRFU2Date)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRFU3Date) Then
                    zCmd.Parameters.Add(New SqlParameter("@colRFU3Date", IIf(pRowData.colRFU3Date Is Nothing, DBNull.Value, pRowData.colRFU3Date)))
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colRRLogGuid) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colCurrentLink) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colLanguagePreference) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colAdminAlertDate) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colAdminAlertReason) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysUserCreated) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysDateCreated) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysUserModified) Then
                ...
                End If
                If zMode = "INSERTwithKey" Or zMode = "INSERTnoKey" Or pRowData.IsDirty(rowTblRRLog.FieldIndex.colSysDateModified) Then
        ...
        Public Shared Function LoadByPrimaryKey(pcolId As Integer) As rowTblRRLog
            Return LoadByPrimaryKey(pcolId, Nothing, Nothing)
        End Function

        Public Shared Function LoadByPrimaryKey(pcolId As Integer, pLoadEventUserState As Object) As rowTblRRLog
            Return LoadByPrimaryKey(pcolId, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByPrimaryKey(pcolId As Integer, pTransaction As SqlClient.SqlTransaction) As rowTblRRLog
            Return LoadByPrimaryKey(pcolId, pTransaction, Nothing)
        End Function

        Public Shared Function LoadByPrimaryKey(pcolId As Integer, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As rowTblRRLog
            Dim zReturn As New rowTblRRLog
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [ID] = @colId"
            Dim zCmd As New SQLCommand
            ...
            Dim zList As List(Of rowTblRRLog) = ReadData(zCmd, pLoadEventUserState)
        ...
        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix As Short) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix As Short, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix As Short, pTransaction As SqlClient.SqlTransaction) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix, pTransaction, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(pcolFilePrefix As Short, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zReturn As New List(Of rowTblRRLog)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [FilePrefix] = @colFilePrefix"
            Dim zCmd As New SQLCommand       
            zCmd.CommandText = zSQL 
            ...
            Return zReturn	
        End Function
        Public Shared Function LoadByIDX_IX_RRGuid(pcolRRLogGuid As String) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_RRGuid(pcolRRLogGuid, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_RRGuid(pcolRRLogGuid As String, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_RRGuid(pcolRRLogGuid, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_IX_RRGuid(pcolRRLogGuid As String, pTransaction As SqlClient.SqlTransaction) As List(Of rowTblRRLog)
            Return LoadByIDX_IX_RRGuid(pcolRRLogGuid, pTransaction, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_RRGuid(pcolRRLogGuid As String, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zReturn As New List(Of rowTblRRLog)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [RRLogGuid] = @colRRLogGuid"
            Dim zCmd As New SQLCommand       
            zCmd.CommandText = zSQL 

            zCmd.Parameters.Add(New SQLParameter("@colRRLogGuid", pcolRRLogGuid))
            ...
            Return zReturn	
        End Function
        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(pcolId As Integer) As List(Of rowTblRRLog)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(pcolId, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(pcolId As Integer, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(pcolId, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(pcolId As Integer, pTransaction As SqlClient.SqlTransaction) As List(Of rowTblRRLog)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(pcolId, pTransaction, Nothing)
        End Function

        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(pcolId As Integer, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zReturn As New List(Of rowTblRRLog)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [ID] = @colId"
            Dim zCmd As New SQLCommand       
            zCmd.CommandText = zSQL 
            ...
            Return zReturn	
        End Function
        Public Shared Function LoadByIDX_uc_rrlog(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String) As List(Of rowTblRRLog)
            Return LoadByIDX_uc_rrlog(pcolFileNo, pcolPracticeCode, pcolHazard, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_uc_rrlog(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Return LoadByIDX_uc_rrlog(pcolFileNo, pcolPracticeCode, pcolHazard, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_uc_rrlog(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pTransaction As SqlClient.SqlTransaction) As List(Of rowTblRRLog)
            Return LoadByIDX_uc_rrlog(pcolFileNo, pcolPracticeCode, pcolHazard, pTransaction, Nothing)
        End Function

        Public Shared Function LoadByIDX_uc_rrlog(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zReturn As New List(Of rowTblRRLog)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [FileNo] = @colFileNo AND [PracticeCode] = @colPracticeCode AND [Hazard] = @colHazard"
            Dim zCmd As New SQLCommand       
            zCmd.CommandText = zSQL 

            zCmd.Parameters.Add(New SQLParameter("@colFileNo", pcolFileNo))
            zCmd.Parameters.Add(New SQLParameter("@colPracticeCode", pcolPracticeCode))
            zCmd.Parameters.Add(New SQLParameter("@colHazard", pcolHazard))
        ...
        Public Shared Function LoadData() As List(Of rowTblRRLog)
            Return LoadData(New SqlClient.SqlCommand(), Nothing, Nothing)
        End Function

        Public Shared Function LoadData(pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Return LoadData(New SqlClient.SqlCommand(), Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadData(pSQL As String) As List(Of rowTblRRLog)
            Dim zCmd As New SqlClient.SqlCommand(pSQL)
            Return LoadData(zCmd, Nothing, Nothing)
        End Function

        Public Shared Function LoadData(pSQL As String, ByRef pTransaction As SqlClient.SqlTransaction) As List(Of rowTblRRLog)
            Dim zCmd As New SqlClient.SqlCommand(pSQL)
            Return LoadData(zCmd, pTransaction, Nothing)
        End Function

        Public Shared Function LoadData(pSQL As String, ByRef pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zCmd As New SqlClient.SqlCommand(pSQL)
        ...
        Public Shared Function LoadData(pCommand As SqlClient.SqlCommand, ByRef pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of rowTblRRLog)
            Dim zReturn As New List(Of rowTblRRLog)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog]"
            Dim zCmd As New SQLCommand
        ...
        Public Shared Function ReadData(pCommand As SqlClient.SqlCommand, pLoadEventUserState As Object) As List(Of rowTblRRLog)

			Try
	            Dim zReturn As New List(Of rowTblRRLog)		
	            Dim zReader As SqlClient.SqlDataReader = razor.UtilSQLServer.ExecuteReader(pCommand)
	            Dim zObj As New rowTblRRLog

	            Using pCommand
	                Using zReader
	                    If zReader.HasRows() Then
	                        While zReader.Read
	                            zObj = New rowTblRRLog		
		...
		Public Shared Function ReadDataRow(pReader As SqlClient.SqlDataReader, ByRef pObject As rowTblRRLog)
    ...
    #Region "Reflection Based Load Functions"
        ...
        Public Shared Function LoadByPrimaryKey(Of T)(pcolId As Integer, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As T
            ...
            If Not TypeOf zReturn Is rowTblRRLog Then Throw New ArgumentException("Generic type is not type of rowTblRRLog.","T")
        ...
        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix As Short) As List(Of T)
            Return LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix As Short, pLoadEventUserState As Object) As List(Of T)
            Return LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix As Short, pTransaction As SqlClient.SqlTransaction) As List(Of T)
            Return LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix, pTransaction, Nothing)
        End Function

        ''' <summary>
        ''' Loads a list of items based on the supplied object type (Reflection Based Function)
        ''' </summary>
        Public Shared Function LoadByIDX_IX_FilePrefix_RRLog(Of T)(pcolFilePrefix As Short, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of T)
            Dim zReturn As New List(Of T)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [FilePrefix] = @colFilePrefix"
            Dim zCmd As New SQLCommand
            zCmd.CommandText = zSQL
        ...
        Public Shared Function LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid As String) As List(Of T)
            Return LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid As String, pLoadEventUserState As Object) As List(Of T)
            Return LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid As String, pTransaction As SqlClient.SqlTransaction) As List(Of T)
            Return LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid, pTransaction, Nothing)
        End Function

        ''' <summary>
        ''' Loads a list of items based on the supplied object type (Reflection Based Function)
        ''' </summary>
        Public Shared Function LoadByIDX_IX_RRGuid(Of T)(pcolRRLogGuid As String, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of T)
            Dim zReturn As New List(Of T)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [RRLogGuid] = @colRRLogGuid"
            Dim zCmd As New SQLCommand
            zCmd.CommandText = zSQL
        ...
        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId As Integer) As List(Of T)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId As Integer, pLoadEventUserState As Object) As List(Of T)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId As Integer, pTransaction As SqlClient.SqlTransaction) As List(Of T)
            Return LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId, pTransaction, Nothing)
        End Function

        ''' <summary>
        ''' Loads a list of items based on the supplied object type (Reflection Based Function)
        ''' </summary>
        Public Shared Function LoadByIDX_PK__RRLog__0E2EFAF4(Of T)(pcolId As Integer, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of T)
            Dim zReturn As New List(Of T)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [ID] = @colId"
            Dim zCmd As New SQLCommand
            zCmd.CommandText = zSQL
        ...
        Public Shared Function LoadByIDX_uc_rrlog(Of T)(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String) As List(Of T)
            Return LoadByIDX_uc_rrlog(Of T)(pcolFileNo, pcolPracticeCode, pcolHazard, Nothing, Nothing)
        End Function

        Public Shared Function LoadByIDX_uc_rrlog(Of T)(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pLoadEventUserState As Object) As List(Of T)
            Return LoadByIDX_uc_rrlog(Of T)(pcolFileNo, pcolPracticeCode, pcolHazard, Nothing, pLoadEventUserState)
        End Function

        Public Shared Function LoadByIDX_uc_rrlog(Of T)(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pTransaction As SqlClient.SqlTransaction) As List(Of T)
            Return LoadByIDX_uc_rrlog(Of T)(pcolFileNo, pcolPracticeCode, pcolHazard, pTransaction, Nothing)
        End Function

        ''' <summary>
        ''' Loads a list of items based on the supplied object type (Reflection Based Function)
        ''' </summary>
        Public Shared Function LoadByIDX_uc_rrlog(Of T)(pcolFileNo As String, pcolPracticeCode As Short, pcolHazard As String, pTransaction As SqlClient.SqlTransaction, pLoadEventUserState As Object) As List(Of T)
            Dim zReturn As New List(Of T)
            Dim zSQL As String = "SELECT * FROM [dbo].[RRLog] WHERE [FileNo] = @colFileNo AND [PracticeCode] = @colPracticeCode AND [Hazard] = @colHazard"
            Dim zCmd As New SQLCommand
            zCmd.CommandText = zSQL
    ...
    End Class
	<serializable>
    Partial Public Class rowTblRRLog
        Implements IrowTblRRLog

        Public Event Loaded(pUserState As Object) Implements IrowTblRRLog.Loaded
        Public Event Saved(pUserState As Object) Implements IrowTblRRLog.Saved
        Public Event Deleted(pUserState As Object) Implements IrowTblRRLog.Deleted


        Public Sub RaiseLoadedEvent(pUserState As Object) Implements IrowTblRRLog.RaiseLoadedEvent
            RaiseEvent Loaded(pUserState)
        End Sub

        Public Sub RaiseDeletedEvent(pUserState As Object) Implements IrowTblRRLog.RaiseDeletedEvent
            RaiseEvent Deleted(pUserState)
        End Sub

        Public Sub RaiseSavedEvent(pUserState As Object) Implements IrowTblRRLog.RaiseSavedEvent
            RaiseEvent Saved(pUserState)
        End Sub
        Public Property colId() As Nullable(Of Integer) Implements IrowTblRRLog.colId
            Get
                Return m_colId
            End Get
            Set
        ...
        Public Property colFilePrefix() As Nullable(Of Short) Implements IrowTblRRLog.colFilePrefix
            Get
        ...
        Public Property colFileNo() As String Implements IrowTblRRLog.colFileNo
            Get
                Return m_colFileNo
            End Get
            Set
        ...
        Public Property colPracticeCode() As Nullable(Of Short) Implements IrowTblRRLog.colPracticeCode
            Get
        ...
        Public Property colHazard() As String Implements IrowTblRRLog.colHazard
            Get
                Return m_colHazard
            End Get
            Set
        ...
        Public Property colDateStamp() As Nullable(Of DateTime) Implements IrowTblRRLog.colDateStamp
            Get
        ...
        Public Property colSender() As String Implements IrowTblRRLog.colSender
            Get
                Return m_colSender
            End Get
            Set
        ...
        Public Property colRecipientName() As String Implements IrowTblRRLog.colRecipientName
            Get
        ...
        Public Property colRecipientEmail() As String Implements IrowTblRRLog.colRecipientEmail
            Get
        ...
        Public Property colActivity() As String Implements IrowTblRRLog.colActivity
            Get
        ...
        Public Property colLinkActive() As Nullable(Of Boolean) Implements IrowTblRRLog.colLinkActive
            Get
        ...
        Public Property colLinkActivatedDate() As Nullable(Of DateTime) Implements IrowTblRRLog.colLinkActivatedDate
            Get
        ...
        Public Property colLinkActivatedBy() As String Implements IrowTblRRLog.colLinkActivatedBy
            Get
        ...
        Public Property colLinkActivatedByEmail() As String Implements IrowTblRRLog.colLinkActivatedByEmail
            Get
        ...
        Public Property colResponseActive() As Nullable(Of Boolean) Implements IrowTblRRLog.colResponseActive
            Get
        ...
        Public Property colResponseLastActivity() As Nullable(Of DateTime) Implements IrowTblRRLog.colResponseLastActivity
            Get
        ...
        Public Property colChanged() As Nullable(Of Boolean) Implements IrowTblRRLog.colChanged
            Get
                Return m_colChanged
            End Get
            Set
        ...
        Public Property colRFU1Date() As Nullable(Of DateTime) Implements IrowTblRRLog.colRFU1Date
            Get
        ...
        Public Property colRFU2Date() As Nullable(Of DateTime) Implements IrowTblRRLog.colRFU2Date
            Get
        ...
        Public Property colRFU3Date() As Nullable(Of DateTime) Implements IrowTblRRLog.colRFU3Date
            Get
        ...
        Public Property colRRLogGuid() As String Implements IrowTblRRLog.colRRLogGuid
            Get
        ...
        Public Property colCurrentLink() As String Implements IrowTblRRLog.colCurrentLink
            Get
        ...
        Public Property colLanguagePreference() As String Implements IrowTblRRLog.colLanguagePreference
            Get
        ...
        Public Property colAdminAlertDate() As Nullable(Of DateTime) Implements IrowTblRRLog.colAdminAlertDate
            Get
        ...
        Public Property colAdminAlertReason() As String Implements IrowTblRRLog.colAdminAlertReason
            Get
        ...
        Public Property colSysUserCreated() As String Implements IrowTblRRLog.colSysUserCreated
            Get
        ...
        Public Property colSysDateCreated() As Nullable(Of DateTime) Implements IrowTblRRLog.colSysDateCreated
            Get
        ...
        Public Property colSysUserModified() As String Implements IrowTblRRLog.colSysUserModified
            Get
        ...
        Public Property colSysDateModified() As Nullable(Of DateTime) Implements IrowTblRRLog.colSysDateModified
            Get
        ...
        Public Function IsDirty(pIndex As rowTblRRLog.FieldIndex) As Boolean Implements IrowTblRRLog.IsDirty
            Return m_IsDirty(pIndex)
        End Function

        Public Function IsDirty() As Boolean Implements IrowTblRRLog.IsDirty
            Return m_IsDirty.Contains(True)
        End Function

        Public Sub SetIsDirty(pDirty As Boolean) Implements IrowTblRRLog.SetIsDirty
        ...
        End Sub
		 ...
		 ''' <summary>
        ''' Returns the type (TblRRLog) that is able to save, update or delete using this row object.
        ''' </summary>
        ''' <returns>Returns the TblpmactivitiesCfc Type</returns>
        ''' <remarks></remarks>
		Public Shared Function GetTableType() As Type
			Dim zObj As New TblRRLog
            Return zObj.GetType
		End Function
    ...
    Public Interface IrowTblRRLog
        ...
        Function IsDirty(pIndex As rowTblRRLog.FieldIndex) As Boolean
