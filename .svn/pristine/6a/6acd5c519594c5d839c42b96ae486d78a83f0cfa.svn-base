...
Public Class tab_facilityrating_boiler
    Inherits System.Web.UI.Page

    Private Sub ValidatePage()
        'CorrectPageCheck(GetGUIDFromQueryString, PageId_FacilityRating_Boiler, PageId_FacilityRating_Boiler)
        CorrectPageCheck(GetGUIDFromQueryString, PageId_LocationDetails_Boiler, PageId_LocationListing_Boiler)
    ...
    Private Sub loadDefaults()
        ...
        'SetBoilerFacilityRatingPractice(zGUID)
        SetBoilerDetailsPractice(zGUID)
        ...
        If Not Request.UrlReferrer Is Nothing Then
            If Request.UrlReferrer.ToString.Length > 0 Then
                If InStr(Request.UrlReferrer.LocalPath.ToLower, "SiteMap/Default.aspx".ToLower) > 0 Then
                    SetValue(zGUID, KeyName.currentGridId, GridId_BMLocationListings, KeyNameGroup.Navigation)
                    SetValue(zGUID, KeyName.NavReturnURL, Request.Url.LocalPath, KeyNameGroup.Session)
                    SetValue(zGUID, KeyName.NavLocationSelectionText, "Please click the Enter link below to select a location to navigate to Facility Rating.", KeyNameGroup.Session)
                    Response.Redirect("~/Home_{{VER}}/Grids/LocationListings/LocationListings.aspx?sid=" & zGUID & "&pLandingPage=facilityrating", False)
                End If
            End If
        End If
