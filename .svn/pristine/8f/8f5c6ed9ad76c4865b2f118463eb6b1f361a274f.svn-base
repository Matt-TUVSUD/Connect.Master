<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="tab_recdatasheet.aspx.vb" Inherits="GRC.Connect.Web.tab_recdatasheet_boiler" %>
    ...
    $(document).ready(function () {
        ...
        var view_RecDatasheet = {
            ...
            loadFileNumber: function () {
                ...
                //if (!view_RecDatasheet.collListing.ViewCCLocationBMRecommendationCollections(tempParm)) {
                if (!view_RecDatasheet.collListing.ViewCCLocationBMCollections(tempParm)) {
                    var tempHemisphere = getHemisphere(calculate_time_zone());
                    es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                    if (!view_RecDatasheet.collRec.viewCCBMRecommendationGridCollection(view_RecDatasheet.gSafeGuid + "|" + tempHemisphere)) {
