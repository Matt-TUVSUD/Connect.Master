<%@ Page Title="Boiler Program Details | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/Home_{{VER}}/ClientConnectServerLocationDetail.Master" CodeBehind="locationdetails.aspx.vb" Inherits="GRC.Connect.Web.locationdetails_boiler" %>
    ...
    <div id="tabLocDetailsContainer" class="tabLocDetailsContainer">
       <div id="tabLocDetailsLoading" class="tabLocDetailsLoading">
         ...
         <div class="choosePracticeHldr">
            ...
            <div id="boilerPracticeLvlDropDown"></div>
        </div>
            ...
            var view = {
                ...
                loadActiveServicesSuccess: function (pResults) {
                    try {
                        if (pResults.isSuccess) {
                            ...
                            if (!view.boilerPracticeLvlDropDown) {
                                $('#boilerPracticeLvlDropDown').kendoDropDownList({
                            ...
                            view.boilerPracticeLvlDropDown = $('#boilerPracticeLvlDropDown').data("kendoDropDownList");
                            view.boilerPracticeLvlDropDown.select(function (dataItem) { return dataItem.value === "51" });
                        }
                    } catch (pException) {
                ...
                serviceDDChange: function (e) {
                    ...
                    switch (chosenServiceValue) {
                        case "51":
                            //B&M
                            tempURL = currentURL.replace("/Boiler/", "/Boiler/");
                        ...
                        case "52":
                            //FIRE
                            tempURL = currentURL.replace("/Boiler/", "/Fire/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "53":
                            //IR
                            tempURL = currentURL.replace("/Boiler/", "/Infrared/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "54":
                            //DIVSERV
                            tempURL = currentURL.replace("/Boiler/", "/Diversified/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "55":
                            //NATHAZ
                            tempURL = currentURL.replace("/Boiler/", "/NaturalHazards/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "56":
                            //JURIS
                            tempURL = currentURL.replace("/Boiler/", "/Jurisdictional/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "57":
                            //PROCSAFETY
                            tempURL = currentURL.replace("/Boiler/", "/ProcessSafety/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
