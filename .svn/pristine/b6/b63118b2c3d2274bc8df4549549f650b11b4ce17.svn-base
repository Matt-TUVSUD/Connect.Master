...
Public Class DefaultNatHaz
    ...
    Private Sub ValidatePage()
        ...
        CorrectPageCheck(zGUID, PageId_LocationListing_NatHaz, PageId_LocationListing_NatHaz)

        'Redirect to location listing grid
        SetValue(zGUID, KeyName.NavLocationSelectionText, "", KeyNameGroup.Session)
        SetValue(zGUID, KeyName.currentGridId, GridId_NaturalHazardsLocationListings, KeyNameGroup.Navigation)
    ...
    Private Sub loadDefaults()
        SetNatHazPractice(GetGUIDFromQueryString)
    End Sub
    ...
    Protected Sub rptFileNo_ItemCommand(source As Object, e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptFileNo.ItemCommand
        ...
        SetValue(zGuid, KeyName.currentTab, PageId_NatHaz, KeyNameGroup.Navigation)
        SetNaturalHazardsDetailsPractice(zGuid)

        Response.Redirect("~/Home_{{VER}}/NaturalHazards/locationdetails.aspx?sid=" & zGuid)
    End Sub
