$(document).ready(function () {
    ...
    /////////////////////////////////
    //// LOAD ALL SITE MAP ITEMS
    var collSiteMapSmall = new es.objects.ViewCCGSafeSitemapItemsCollection();
    var collSiteMap = new es.objects.ViewCCGSafeSitemapItemsCollection();
    es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
    if (!collSiteMap.GetAllSiteMapItems(gSafeGuid)) {
    } else {
        alert("Error: Unable to load site map links.");
    }
    ...
    //// BUILD DATASOURCES
    //// MAIN MENU
    parms = "MainMenuNavigation";
    dsMainMenuNavigation = buildDataSource(parms);
    parms = "MainMenuQuickRef";
    dsMainMenuQuickRef = buildDataSource(parms);
    parms = "MainMenuLibrary";
    dsMainMenuLibrary = buildDataSource(parms);

    //// FIRE
    parms = "FireLocationListing";
    dsFireLocationListing = buildDataSource(parms);
    parms = "FireOutputReports";
    dsFireOutputReports = buildDataSource(parms);
    parms = "FireDataExtracts";
    dsFireDataExtracts = buildDataSource(parms);

    //// BOILER
    parms = "BMLocationListing";
    dsBMLocationListing = buildDataSource(parms);
    parms = "BMOutputReports";
    dsBMOutputReports = buildDataSource(parms);
    parms = "BMDataExtracts";
    dsBMDataExtracts = buildDataSource(parms);

    //// INFRARED
    parms = "IRLocationListing";
    dsIRLocationListing = buildDataSource(parms);
    parms = "IROutputReports";
    dsIROutputReports = buildDataSource(parms);
    parms = "IRDataExtracts";
    dsIRDataExtracts = buildDataSource(parms);

    //// JURIS
    parms = "JurisLocationListing";
    dsJurisLocationListing = buildDataSource(parms);
    parms = "JurisOutputReports";
    dsJurisOutputReports = buildDataSource(parms);
    parms = "JurisDataExtracts";
    dsJurisDataExtracts = buildDataSource(parms);

    //// SEISMIC
    parms = "NatHazSeismicLocationListing";
    dsNatHazSeismicLocationListing = buildDataSource(parms);
    parms = "NatHazSeismicOutputReports";
    dsNatHazSeismicOutputReports = buildDataSource(parms);
    parms = "NatHazSeismicDataExtracts";
    dsNatHazSeismicDataExtracts = buildDataSource(parms);

    //// FLOOD
    parms = "NatHazFloodLocationListing";
    dsNatHazFloodLocationListing = buildDataSource(parms);
    parms = "NatHazFloodOutputReports";
    dsNatHazFloodOutputReports = buildDataSource(parms);
    parms = "NatHazFloodDataExtracts";
    dsNatHazFloodDataExtracts = buildDataSource(parms);

    //// WIND
    parms = "NatHazWindLocationListing";
    dsNatHazWindLocationListing = buildDataSource(parms);
    parms = "NatHazWindOutputReports";
    dsNatHazWindOutputReports = buildDataSource(parms);
    parms = "NatHazWindDataExtracts";
    dsNatHazWindDataExtracts = buildDataSource(parms);
    ...
    var cell0301Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0301", 3, "left", "bottom", "0px", "500px"); cell0301TreeView.expand(".k-item"); },
    ...
    var cell0302Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0302", 3, "middle", "bottom", "0px", "500px"); cell0302TreeView.expand(".k-item"); },
    ...
    var cell0303Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0303", 3, "middle", "bottom", "0px", "500px"); cell0303TreeView.expand(".k-item"); },
    ...
    var cell0304Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0304", 3, "middle", "bottom", "0px", "500px"); cell0304TreeView.expand(".k-item"); },
    ...
    var cell0305Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0305", 3, "middle", "bottom", "0px", "500px"); cell0305TreeView.expand(".k-item"); },
    ...
    var cell0306Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0306", 3, "middle", "bottom", "0px", "500px"); cell0306TreeView.expand(".k-item"); },
    ...
    var cell0307Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0307", 3, "middle", "bottom", "0px", "500px"); cell0307TreeView.expand(".k-item"); },
    ...
    var cell0308Config = {
        interval: 300,
        over: function (event) { zoomIn("#cell0308", 3, "right", "bottom", "0px", "500px"); cell0308TreeView.expand(".k-item"); },
    ...
    function zoomIn(cell, currentRow, hAlign, vAlign, marginTop, newContentHeight) {
        if (animationFlag) {
            ...
            else if (vAlign == "bottom") { mTop = "-350px"; }
    ...
    function buildDataSource(keyword) {
        //// CLEAR THE SMALL ARRAY GET ALL ITEMS THAT ARE EQUAL TO THE KEYWORD
        //// MAKES FOR FASTER ARRAY BUILDING BELOW
        collSiteMapSmall = new es.objects.ViewCCGSafeSitemapItemsCollection();
        for (z = 0; z < collSiteMap().length; z++) {
            if (collSiteMap()[z].NavSiteMapKeyWord() == keyword) {
                collSiteMapSmall().push(collSiteMap()[z]);
            }
        }
        ...
        var targetAutoRec = "AutoRecommendationResponseFollowUpStatus";
        var targetCharts = "/Charts/";

        for (y = 0; y < collSiteMapSmall().length; y++) {
            if (collSiteMapSmall()[y].NavParentLinkId() != undefined && collSiteMapSmall()[y].NavParentLinkId() == parentId) {
                //// GET CHILD NODES
                var subMenu = buildSubMenu(collSiteMapSmall()[y].NavLinkId());
                //// GET THE TARGET OF THE NODE
                var tempStr = collSiteMapSmall()[y].NavUrl();
                var tempTarget = "";
                if (tempStr != undefined) {
                    ...
                    else if (tempStr.search(targetAutoRec) > -1) { tempTarget = "recfollowupstatus" }
                    else if (tempStr.search(targetCharts) > -1) { tempTarget = "Charts" }
                    else { tempTarget = collSiteMapSmall()[y].NavTarget() + "" }
                }

                //// BUILD THE NODE
                if (returnCollection.length == 0) {
                    returnCollection = [{
                        key: collSiteMapSmall()[y].NavLinkId(),
                        text: collSiteMapSmall()[y].NavText(),
                        url: getNavUrl(collSiteMapSmall()[y].NavUrl(), collSiteMapSmall()[y].NavEnabled()),
                        navEnabled: collSiteMapSmall()[y].NavEnabled(),
                        //type: collSiteMapSmall()[y].NavType().toLowerCase(),
                        items: subMenu,
                        target: tempTarget,
                        title: "(NavLinkId: " + collSiteMapSmall()[y].NavLinkId() + ", GridId: " + collSiteMapSmall()[y].NavGridId() + ", ChartId: " + collSiteMapSmall()[y].NavChartId() + ", ReportCode: " + collSiteMapSmall()[y].NavRptCode() + ")",
                        comment: "(NavLinkId: " + collSiteMapSmall()[y].NavLinkId() + ", GridId: " + collSiteMapSmall()[y].NavGridId() + ", ChartId: " + collSiteMapSmall()[y].NavChartId() + ", ReportCode: " + collSiteMapSmall()[y].NavRptCode() + ") " + collSiteMapSmall()[y].NavStatusToolTip(),
                        chartId: collSiteMapSmall()[y].NavChartId(),
                        gridId: collSiteMapSmall()[y].NavGridId(),
                        reportId: collSiteMapSmall()[y].NavRptCode()
                    }];
                } else {
    ...
    function buildSubMenu(NavLinkId) {
        ...
        for (x = 0; x < collSiteMapSmall().length; x++) {
            if (NavLinkId == collSiteMapSmall()[x].NavParentLinkId()) {
                //// GET THE TARGET OF THE NODE
                var tempChildStr = collSiteMapSmall()[x].NavUrl();
                var tempChildTarget = "";
                if (tempChildStr != undefined) {
                    ...
                    else { tempChildTarget = collSiteMapSmall()[x].NavTarget() + "" }
                }

                if (returnSubMenu.length == 0) {
                    returnSubMenu = [{
                        key: collSiteMapSmall()[x].NavLinkId(),
                        text: collSiteMapSmall()[x].NavText(),
                        url: getNavUrl(collSiteMapSmall()[x].NavUrl(), collSiteMapSmall()[x].NavEnabled()),
                        navEnabled: collSiteMapSmall()[x].NavEnabled(),
                        //type: collSiteMapSmall()[x].NavType().toLowerCase(),
                        target: tempChildTarget,
                        title: "(NavLinkId: " + collSiteMapSmall()[x].NavLinkId() + ", GridId: " + collSiteMapSmall()[x].NavGridId() + ", ChartId: " + collSiteMapSmall()[x].NavChartId() + ", ReportCode: " + collSiteMapSmall()[x].NavRptCode() + ")",
                        comment: "(NavLinkId: " + collSiteMapSmall()[x].NavLinkId() + ", GridId: " + collSiteMapSmall()[x].NavGridId() + ", ChartId: " + collSiteMapSmall()[x].NavChartId() + ", ReportCode: " + collSiteMapSmall()[x].NavRptCode() + ") " + collSiteMapSmall()[x].NavStatusToolTip(),
                        chartId: collSiteMapSmall()[x].NavChartId(),
                        gridId: collSiteMapSmall()[x].NavGridId(),
                        reportId: collSiteMapSmall()[x].NavRptCode()
                }];
                } else {
                    returnSubMenu.push({
                        key: collSiteMapSmall()[x].NavLinkId(),
    ...
    function getParentId() {
        for (x = 0; x < collSiteMapSmall().length; x++) {
            if (collSiteMapSmall()[x].NavParentLinkId() == 0 || collSiteMapSmall()[x].NavParentLinkId() == undefined) {
                return collSiteMapSmall()[x].NavLinkId();
            }
        }
    }

    function getNavUrl(navUrl, isEnabled) {
