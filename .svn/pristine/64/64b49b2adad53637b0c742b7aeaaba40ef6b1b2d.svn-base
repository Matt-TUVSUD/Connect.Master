...
Public Class recdatasheetfeaturesL_nathaz_recresponse
    ...
    Public ServerSide_ToNHReport As String = ""
    Public ServerSide_ToNHDiagram As String = ""
    ...
    Private Sub loadData()
        ...
        'GET NatHaz REPORTS AND DIAGRAM
        Dim zFileNo As String = ServerSide_FileNo
        Dim zObjListing As New ViewCCLocationListingNH
        Dim zQryListing As New ViewCCLocationListingNHQuery
        zQryListing.Select()
        zQryListing.es.Top = 1
        zQryListing.Where(zQryListing.FileNo = zFileNo)
        If zObjListing.Load(zQryListing) Then
            ServerSide_ToNHReport = Replace(zObjListing.ToNHReport, "\", "\\")
            ServerSide_ToNHDiagram = Replace(zObjListing.ToNHDiagram, "\", "\\")
        End If
