...
Public Class DefaultProcessSafety
    ...
    Private Sub ValidatePage()
        ...
        CorrectPageCheck(zGUID, PageId_LocationListing_PS, PageId_LocationListing_PS)

        'Redirect to location listing grid
        SetValue(zGUID, KeyName.NavLocationSelectionText, "", KeyNameGroup.Session)
        SetValue(zGUID, KeyName.currentGridId, GridId_PSLocationListings, KeyNameGroup.Navigation)
    ...
    Private Sub loadDefaults()
        SetPSPractice(GetGUIDFromQueryString)
    End Sub
    ...
    Protected Sub rptFileNo_ItemCommand(source As Object, e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptFileNo.ItemCommand
        ...
        SetValue(zGuid, KeyName.currentTab, PageId_PS, KeyNameGroup.Navigation)
        SetPSPractice(zGuid)

        Response.Redirect("~/Home_{{VER}}/ProcessSafety/locationdetails.aspx?sid=" & zGuid)
    End Sub
