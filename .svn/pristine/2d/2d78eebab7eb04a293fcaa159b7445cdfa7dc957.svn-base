<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="tab_recdatasheet.aspx.vb" Inherits="GRC.Connect.Web.tab_recdatasheet_infared" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var view_RecDatasheet = {
            ...
            onExport: function () {
                ...
                if (view_RecDatasheet.clientFeatures().indexOf("Q") > -1) {
                    ...
                    Content = buildXMLString_InfraredMgmt(gridRecDatasheet.dataSource._view, view_RecDatasheet.objListing());
                    Footer = buildXMLRecFooter();
                } else {
                    Header = buildXMLRecHeader_Infrared();
                    Content = buildXMLString_Infrared(gridRecDatasheet.dataSource._view, view_RecDatasheet.objListing());
                    Footer = buildXMLRecFooter();
                }
            ...
            buildGrid: function () {
                ...
                for (y = 0; y < view_RecDatasheet.collRec().length; y++) {
                    view_RecDatasheet.dataRowAdded(false);
                    if (view_RecDatasheet.hasBothMGMTAndThirdFields()) {
                        if (!view_RecDatasheet.dataRowAdded()) {
                            if (view_RecDatasheet.collRec()[y].FileNo() == view_RecDatasheet.objListing().FileNo()) {
                                view_RecDatasheet.collGrid().push({
                                    ...
                                    RecMonth: view_RecDatasheet.collRec()[y].RecMonth(),
                                    RecNumber: view_RecDatasheet.collRec()[y].RecNumber(),
                                    RecYear: view_RecDatasheet.collRec()[y].RecYear(),
                                    Severityrating: view_RecDatasheet.collRec()[y].Severityrating(),
                                    Recommendation: view_RecDatasheet.collRec()[y].Recommendation(),
                                    RecommendationID: view_RecDatasheet.collRec()[y].RecommendationID(),
                                    FindEquip: view_RecDatasheet.collRec()[y].FindEquip(),
                                    Itemno: view_RecDatasheet.collRec()[y].Itemno(),
                                    Findingno: view_RecDatasheet.collRec()[y].Findingno(),
                                    PlantAreaAffected: view_RecDatasheet.collRec()[y].PlantAreaAffected(),
                                    FaultType: view_RecDatasheet.collRec()[y].FaultType(),
                                    ProbableEquipmentDamageCost: view_RecDatasheet.collRec()[y].ProbableEquipmentDamageCost(),
                                    LengthOfBusinessInterruption: view_RecDatasheet.collRec()[y].LengthOfBusinessInterruption(),
                                    CostOfBusinessInterruption: view_RecDatasheet.collRec()[y].CostOfBusinessInterruption(),
                                    DetectedWith: view_RecDatasheet.collRec()[y].DetectedWith(),
                                    Impact: view_RecDatasheet.collRec()[y].Impact(),
                                    Comments2: view_RecDatasheet.collRec()[y].Comments2(),
                                    RecStatus: view_RecDatasheet.collRec()[y].RecStatus(),
                                    StatusDate: view_RecDatasheet.collRec()[y].StatusDate(),
                                    ResponseFrom: view_RecDatasheet.collRec()[y].ResponseFrom(),
                                    ResponseDate: view_RecDatasheet.collRec()[y].ResponseDate(),
                                    ResponseComment: view_RecDatasheet.collRec()[y].ResponseComment(),
                                    ResponseStatus: view_RecDatasheet.collRec()[y].ResponseStatus(),
                                    RedundancySpare: view_RecDatasheet.collRec()[y].RedundancySpare(),
                                    RecPrimary: view_RecDatasheet.collRec()[y].RecPrimary(),
                                    RecSecondary: view_RecDatasheet.collRec()[y].RecSecondary(),
                                    IntendedAction: view_RecDatasheet.collRec()[y].IntendedAction(),
                                    PlantComment: view_RecDatasheet.collRec()[y].PlantComment(),
                                    Id: view_RecDatasheet.collRec()[y].Id(),
                                    History: view_RecDatasheet.collRec()[y].History(),
                                    Comments: view_RecDatasheet.collRec()[y].Comments(),
                                    ExpectedCompletionDate: view_RecDatasheet.collRec()[y].ExpectedCompletionDate(),
                                    ...
                                    ThirdLevelRespDate: view_RecDatasheet.collRec()[y].ThirdLevelRespDate(),
                                    ThirdLevelRespAction: view_RecDatasheet.collRec()[y].ThirdLevelRespAction(),
                                    ThirdLevelRespFrom: view_RecDatasheet.collRec()[y].ThirdLevelRespFrom(),
                                    ThirdLevelRespComment: view_RecDatasheet.collRec()[y].ThirdLevelRespComment()
                                });
                                view_RecDatasheet.dataRowAdded(true);
                            }
                        }
                    }

                    if (view_RecDatasheet.hasMGMTFieldsOnly()) {
                        if (view_RecDatasheet.collRec()[y].FileNo() == view_RecDatasheet.objListing().FileNo()) {
                            view_RecDatasheet.collGrid().push({
                                IdWeb: y,
                                FileNo: view_RecDatasheet.collRec()[y].FileNo(),
                    ...
                    if (view_RecDatasheet.hasMagnaFields()) {
                        if (view_RecDatasheet.collRec()[y].FileNo() == view_RecDatasheet.objListing().FileNo()) {
                            view_RecDatasheet.collGrid().push({
                                IdWeb: y,
                                FileNo: view_RecDatasheet.collRec()[y].FileNo(),
                    ...
                    if (view_RecDatasheet.hasThirdFieldsOnly()) {
                        if (!view_RecDatasheet.dataRowAdded()) {
                            if (view_RecDatasheet.collRec()[y].FileNo() == view_RecDatasheet.objListing().FileNo()) {
                                view_RecDatasheet.collGrid().push({
                                    IdWeb: y,
                    ...
                    if (view_RecDatasheet.hasNoClientFeatures()) {
                        if (!view_RecDatasheet.dataRowAdded()) {
                            if (view_RecDatasheet.collRec()[y].FileNo() == view_RecDatasheet.objListing().FileNo()) {
                                view_RecDatasheet.collGrid().push({
                                    FileNo: view_RecDatasheet.collRec()[y].FileNo(),
            ...
            loadFileNumber: function () {
                var tempParm = view_RecDatasheet.gSafeGuid + "|" + view_RecDatasheet.fileDirection();
                es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                //if (!view_RecDatasheet.collListing.ViewCCLocationIRRecommendationCollections(tempParm)) {
                if (!view_RecDatasheet.collListing.ViewCCLocationIRCollections(tempParm)) {
                    var tempHemisphere = getHemisphere(calculate_time_zone());
                    es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                    if (!view_RecDatasheet.collRec.viewCCIRRecommendationGridCollection(view_RecDatasheet.gSafeGuid + "|" + tempHemisphere)) {
                        ...
                        view_RecDatasheet.objFind(getFindLocationFields(view_RecDatasheet.gSafeGuid));
                            if (view_RecDatasheet.clientFeatures().indexOf("Q") > -1) {
                                if (view_RecDatasheet.clientFeatures().indexOf("Q") > -1 && view_RecDatasheet.clientFeatures().indexOf("@") > -1) {
                                    view_RecDatasheet.hasBothMGMTAndThirdFields(true);
                                } else {
                                    view_RecDatasheet.hasMGMTFieldsOnly(true);
                                }
                            } else {
                                view_RecDatasheet.hasMGMTFieldsOnly(false);
                            }

                            if (view_RecDatasheet.clientFeatures().indexOf("U") > -1) {
                                view_RecDatasheet.hasMagnaFields(true);
                            } else {
                                view_RecDatasheet.hasMagnaFields(false);
                            }

                            if (view_RecDatasheet.clientFeatures().indexOf("@") > -1) {
                                if (view_RecDatasheet.clientFeatures().indexOf("@") > -1 && view_RecDatasheet.clientFeatures().indexOf("Q") > -1) {
                                    view_RecDatasheet.hasBothMGMTAndThirdFields(true);
                                } else {
                                    view_RecDatasheet.hasThirdFieldsOnly(true);
                                }
                            } else {
                                view_RecDatasheet.hasThirdFieldsOnly(false);
                            }
                            if (view_RecDatasheet.clientFeatures().indexOf("@") > -1 || view_RecDatasheet.clientFeatures().indexOf("Q") > -1 || view_RecDatasheet.clientFeatures().indexOf("U") > -1) {
                                view_RecDatasheet.hasNoClientFeatures(false);
                            } else {
                                view_RecDatasheet.hasNoClientFeatures(true);
                            }
        ...
        var dsRec;
        if (view_RecDatasheet.clientFeatures().length > 0) {
            if (view_RecDatasheet.hasNoClientFeatures()) {
            ...
            if (view_RecDatasheet.hasBothMGMTAndThirdFields()) {
                gridRecDatasheet = $("#gridRecDatasheet").kendoGrid({
                    ...
                    columns: [
                        ...
                        { field: "MgmtRespComment", width: "450px", title: "Mgmt. Comment", template: "#= MgmtRespComment #" },
                        ...
                        { field: "ThirdLevelRespComment", width: "450px", title: "3rd Lvl Comment", template: "#= ThirdLevelRespComment #" }
                    ]
                }).data("kendoGrid");
            }
            if (view_RecDatasheet.hasMGMTFieldsOnly()) {
    ...
    function onEnter(e) {
        ...
        var tempUrl = "tab_rectracking.aspx?sid=" + getGSafeGUID() + "&rec=" + dataItem.IdWeb;
        //document.location.href = tempUrl;
        gReloadTab(tempUrl, dataItem.IdWeb);
    }
