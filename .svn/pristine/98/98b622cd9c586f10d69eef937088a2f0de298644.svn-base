<%@ Page Title="Jurisdictional Program Details | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/Home_{{VER}}/ClientConnectServerLocationDetail.Master" CodeBehind="locationdetails.aspx.vb" Inherits="GRC.Connect.Web.locationdetails_juris" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div id="tabLocDetailsContainer" class="tabLocDetailsContainer">
        ...
        <div class="choosePracticeHldr">
            ...
            <div id="jurisPracticeLvlDropDown"></div>
        </div>
            ...
            var view = {
                ...
                loadPages: function () {
                    try {
                        ...
                        view.urlDataSource = [
                            { "id": 1, "label": "Recommendations", "content": view.urlLocationDetails + view.urlParameters }
                        ];
                    } catch (pException) {
                ...
                loadActiveServicesSuccess: function (pResults) {
                    try {
                        if (pResults.isSuccess) {
                            ...
                            if (!view.jurisPracticeLvlDropDown) {
                                $('#jurisPracticeLvlDropDown').kendoDropDownList({
                            ...
                            view.jurisPracticeLvlDropDown = $('#jurisPracticeLvlDropDown').data("kendoDropDownList");
                            view.jurisPracticeLvlDropDown.select(function (dataItem) { return dataItem.value === "56" });
                        }
                    } catch (pException) {
                ...
                serviceDDChange: function (e) {
                    ...
                    switch (chosenServiceValue) {
                        case "51":
                            //B&M
                            tempURL = currentURL.replace("/Jurisdictional/", "/Boiler/");
                        ...
                        case "52":
                            //FIRE
                            tempURL = currentURL.replace("/Jurisdictional/", "/Fire/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "53":
                            //IR
                            tempURL = currentURL.replace("/Jurisdictional/", "/Infrared/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "54":
                            //DIVSERV
                            tempURL = currentURL.replace("/Jurisdictional/", "/Diversified/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "55":
                            //NATHAZ
                            tempURL = currentURL.replace("/Jurisdictional/", "/NaturalHazards/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "56":
                            //JURIS
                            tempURL = currentURL.replace("/Jurisdictional/", "/Jurisdictional/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
                        case "57":
                            //PROCSAFETY
                            tempURL = currentURL.replace("/Jurisdictional/", "/ProcessSafety/");
                            tempParm = "p1=" + tempFileNo;
                            tempURL = tempURL.replace(oldParm, tempParm);
                            window.location.href = tempURL;
                            break;
