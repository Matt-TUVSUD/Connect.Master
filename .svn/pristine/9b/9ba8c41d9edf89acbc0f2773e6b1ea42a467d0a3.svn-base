<%@ Page Title="Rec. Response Grid | Global Risk Consultants" Language="vb" AutoEventWireup="false" MasterPageFile="~/RecResponse_{{VER}}/ClientConnectRecResponse.Master" CodeBehind="recdatasheetfeaturesL.aspx.vb" Inherits="GRC.Connect.Web.recdatasheetfeaturesL_seismic_recresponse" %>
    ...
    $(document).ready(function () {
        ...
        var view = {
            ...
            loadFileNumber: function () {
                ...
                es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                if (!view.collListing.GetRecResponseNatHazLocationByFileNo(tempParm)) {
                    var tempHemisphere = getHemisphere(calculate_time_zone());
                    es.dataProvider.baseURL = SITEBASEURL + VIEWSERVERENDPOINT;
                    if (!view.collRec.GetSeismicRecommendationGridByFileNo(tempParm + "|" + tempHemisphere)) {
