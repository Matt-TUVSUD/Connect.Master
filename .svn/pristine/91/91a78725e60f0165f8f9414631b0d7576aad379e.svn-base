    ...
    var viewRecs = {
        ...
        loadRecommendationsData: function () {
            ...
            if (viewRecs.recommendationsDataEndPointUrl() == undefined) {
                var tempEndpoint = viewRecs.endPointBase + "clients/" + viewRecs.endPointFilePrefix() + "/recs?recType=" + viewRecs.currentRecType() + "&fileNo=" + viewRecs.currentFilePrefix() + "&langid=" + viewRecs.langID();
        ...
        //    viewRecs.recommendationsDataEndPointUrl();
        ...
        //                debugger;
        //                for (var i = 0; i < pResults.data.length; i++) {
        //                    if (pResults.data[i].fileNo == viewRecs.currentRec().fileNo) {
        ...
        loadDropDownData: function () {
             ...
             * 6=MgmtRecActions
             * */
            var endPointURL = viewRecs.endPointBase + "meta/lists?type=1&type=2&type=3&langid=" + viewRecs.langID();
            if (viewRecs.showMGMT() && !viewRecs.show3rdLvl()) {
                endPointURL = viewRecs.endPointBase + "meta/lists?type=1&type=2&type=3&type=6&langid=" + viewRecs.langID();
            }
            if (viewRecs.showMGMT() && viewRecs.show3rdLvl()) {
                endPointURL = viewRecs.endPointBase + "meta/lists?type=1&type=2&type=3&type=6&type=10&langid=" + viewRecs.langID();
            }
            if (!viewRecs.showMGMT() && viewRecs.show3rdLvl()) {
                endPointURL = viewRecs.endPointBase + "meta/lists?type=1&type=2&type=3&type=10&langid=" + viewRecs.langID();
            }
            $.ajax({
                type: "GET",
                dataType: "json",
        ...
        populateSaveObject: function () {
            viewRecs.currentRec().status = $('#recStatusDD').val();
            ...
            //viewRecs.currentRec().costToComplete = global.formatMoneyNoSymbol(parseInt($('#facilityActCostComp').val()), 0);
            viewRecs.currentRec().costToComplete = parseInt($('#facilityActCostComp').val());
        ...
        saveRecFormSuccess: function () {
            ...
            //viewRecs.loadRecommendationsData();
            //viewRecs.bindRecForm(viewRecs.currentRec().fileNo, viewRecs.currentRec().rowGuid);
            viewRecs.pagerBindRecForm(viewRecs.currentRec().rowGuid);
        ...
        intendedActionSelect: function (e) {
            ...
            console.log(dataItem.displayText);
